...
const LihatWO = () => {
  ...
  const getKorlapsForWorkzone = useCallback(
    (workzone) => {
      if (!workzone) return [];
      const match = workzoneMap.find((m) => m.workzone === workzone);
      const korlapString = match
        ? match.korlaps || match.korlap_username || ""
        : "";
      return korlapString
        .split(",")
        .map((k) => k.trim())
  ...
  const {
    ...
    const allSektors = [
      ...new Set(workzoneMap.map((item) => item.sektor).filter(Boolean)),
    ].sort();
    const allWorkzones = [
      ...new Set(workzoneMap.map((item) => item.workzone).filter(Boolean)),
    ].sort();
    ...
    const allKorlaps = [
      ...new Set(
        workzoneMap.flatMap((item) => getKorlapsForWorkzone(item.workzone))
      ),
    ].sort();
  ...
  const handleUpdateRow = useCallback(
    async (originalItem, updatedFields) => {
      ...
      if ("workzone" in updatedFields) {
        ...
        dataToSend.korlap = match
          ? match.korlaps || match.korlap_username || null
          : null;
      }
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response
            .json()
            .catch(() => ({
              message: "Gagal menyimpan, respons server tidak valid",
            }));
  ...
  const handleBulkComplete = useCallback(async () => {
    ...
    if (
      window.confirm(
        `Yakin ingin menyelesaikan ${selectedItems.length} tiket terpilih?`
      )
    ) {
      const ticketsToComplete = woData.filter((item) =>
        selectedItems.includes(item.incident)
      );
      const invalidTickets = ticketsToComplete.filter((item) => !item.sektor);

      if (invalidTickets.length > 0) {
        alert(
          `Gagal! ${invalidTickets.length} tiket belum memiliki Sektor. Selesaikan tiket ini satu per satu.`
        );
        return;
      }
      ...
      try {
        const promises = selectedItems.map((id) =>
          fetch(`${API_BASE_URL}/work-orders/${id}/complete`, {
            method: "POST",
          })
        );
        ...
        alert(
          `Terjadi kesalahan saat menyelesaikan tiket secara massal: ${err.message}`
        );
      }
    }
  ...
  const handleRemoveDuplicates = useCallback(() => {
    if (
      window.confirm(
        "Yakin ingin menghapus data duplikat (baru) yang data aslinya sudah CLOSED?"
      )
    ) {
      ...
      const incidentsToDelete = woData
        ...
        .filter((newTicket) => {
          ...
          const original = originalTickets.find(
            (old) =>
              old.service_id === newTicket.service_id &&
              old.customer_id === newTicket.customer_id
          );
      ...
      if (incidentsToDelete.length > 0) {
        ...
        Promise.all(
          ...
          .then((responses) => {
            ...
            setWoData((prev) =>
              prev.filter((item) => !incidentsToDelete.includes(item.incident))
            );
            alert(`${incidentsToDelete.length} data duplikat telah dihapus.`);
          })
          .catch((err) =>
            alert(`Gagal menghapus item duplikat: ${err.message}`)
          );
      } else {
        alert(
          "Tidak ditemukan data duplikat yang bisa dihapus (pastikan data asli/lama sudah berstatus CLOSED)."
        );
      }
    }
  }, [woData]);
    ...
    <div className="lihat-wo-container">
      ...
      <div className="table-controls">
        ...
        <div className="action-section">
          ...
          >
            Selesaikan ({selectedItems.length})
          </button>
          <button
          ...
          <button onClick={handleRemoveDuplicates} className="btn btn-warning">
            Hapus Duplikat
          </button>
        </div>
      </div>
            ...
            {dataToShow.length === 0 ? (
              ...
              dataToShow.map((item) => (
                <WorkOrderRow
                  ...
                  statusOptions={ALL_STATUS_OPTIONS.map((opt) => ({
                    label: opt,
                    value: opt,
                  }))}
