...
const LihatWO = () => {
  ...
  const handleRemoveDuplicates = useCallback(() => {
    if (
      window.confirm(
        "Yakin ingin menghapus data duplikat berdasarkan Service ID dan Customer ID? Data yang terakhir diubah akan dipertahankan."
      )
    ) {
      ...
      if (duplicates.length > 0) {
        ...
        Promise.all(
          ...
          .then((responses) => {
            ...
            setWoData((prev) =>
              prev.filter((item) => !duplicateIncidents.includes(item.incident))
            );
